document.addEventListener('DOMContentLoaded', () => {
    const createRain = () => {
        ...
        if (!rainContainer) {
            console.error('Rain animation container not found.');
            return;
        }
    ...
    if (testimonials.length === 0) {
        console.error('No testimonials found in the DOM.');
        return;
    }
    ...
    function showTestimonial(index) {
        testimonials.forEach((testimonial, i) => {
            ...
            if (i === index) {
                testimonial.style.visibility = 'visible'; // Make visible
                testimonial.style.opacity = '1'; // Fully visible
                setTimeout(() => testimonial.classList.add('active'), 10); // Add active class for animation
            } else if (testimonial.style.visibility === 'visible') {
                testimonial.classList.add('exit'); // Add exit class for sliding out
                setTimeout(() => {
                    testimonial.style.visibility = 'hidden'; // Hide after animation
                    testimonial.style.opacity = '0'; // Fully hidden
